...
int Size;
float *a_uni_cuda, *b_uni_cuda, *finalVec;
float *m_uni_cuda;
...
void ForwardSub();
void BackSub();
__global__ void Fan1(float *m_uni_cuda, float *a_uni_cuda, int Size, int t);
__global__ void Fan2(float *m_uni_cuda, float *a_uni_cuda, float *b_uni_cuda,int Size, int j1, int t);
  ...
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
    ...
    for(i=1;i<argc;i++) {
      if (argv[i][0]=='-') {// flag
        ...
        cudaMallocManaged((void **) &m_uni_cuda, Size * Size * sizeof(float));
        cudaMallocManaged((void **) &a_uni_cuda, Size * Size * sizeof(float));
	      cudaMallocManaged((void **) &b_uni_cuda, Size * sizeof(float));
          	// // allocate unified memory 
	
	 

	      //a = (float *) malloc(Size * Size * sizeof(float));
	      create_matrix(a_uni_cuda, Size);
    ...
    if (verbose) {
        printf("Matrix m is: \n");
        PrintMat(m_uni_cuda, Size, Size);

        printf("Matrix a is: \n");
        PrintMat(a_uni_cuda, Size, Size);

        printf("Array b is: \n");
        PrintAry(b_uni_cuda, Size);
    }
    BackSub();
    if (verbose) {
...
void InitProblemOnce(char *filename)
{
	...
	InitMat(a_uni_cuda, Size, Size);
	//printf("The input matrix a is:\n");
	//PrintMat(a, Size, Size);
	// b = (float *) malloc(Size * sizeof(float));
	
	InitAry(b_uni_cuda, Size);
...
{   
	...
	*(m_uni_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t) = *(a_uni_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t) / *(a_uni_cuda+Size*t+t);
}
...
__global__ void Fan2(float *m_uni_cuda, float *a_uni_cuda, float *b_uni_cuda,int Size, int j1, int t)
{
	...
	a_uni_cuda[Size*(xidx+1+t)+(yidx+t)] -= m_uni_cuda[Size*(xidx+1+t)+t] * a_uni_cuda[Size*t+(yidx+t)];
	//a_cuda[xidx+1+t][yidx+t] -= m_cuda[xidx+1+t][t] * a_cuda[t][yidx+t];
	if(yidx == 0){
		//printf("blockIdx.x:%d,threadIdx.x:%d,blockIdx.y:%d,threadIdx.y:%d,blockDim.x:%d,blockDim.y:%d\n",blockIdx.x,threadIdx.x,blockIdx.y,threadIdx.y,blockDim.x,blockDim.y);
		//printf("xidx:%d,yidx:%d\n",xidx,yidx);
		b_uni_cuda[xidx+1+t] -= m_uni_cuda[Size*(xidx+1+t)+(yidx+t)] * b_uni_cuda[t];
	}
}
...
{
	int t;
    // float *m_uni_cuda,*a_uni_cuda,*b_uni_cuda;
    ...
    gettimeofday(&time_start, NULL);
	for (t=0; t<(Size-1); t++) {
		Fan1<<<dimGrid,dimBlock>>>(m_uni_cuda,a_uni_cuda,Size,t);
		cudaThreadSynchronize();
		Fan2<<<dimGridXY,dimBlockXY>>>(m_uni_cuda,a_uni_cuda,b_uni_cuda,Size,Size-t,t);
		cudaThreadSynchronize();
		checkCUDAError("Fan2");
	}
	// end timing kernels
	...
	// cudaMemcpy(m, m_uni_cuda, Size * Size * sizeof(float),cudaMemcpyDeviceToHost );
	// cudaMemcpy(a, a_uni_cuda, Size * Size * sizeof(float),cudaMemcpyDeviceToHost );
	// cudaMemcpy(b, b_uni_cuda, Size * sizeof(float),cudaMemcpyDeviceToHost );
...
void BackSub()
{
	...
	int i,j;
	for(i=0;i<Size;i++){
		finalVec[Size-i-1]=b_uni_cuda[Size-i-1];
		for(j=0;j<i;j++)
		{
			finalVec[Size-i-1]-=*(a_uni_cuda+Size*(Size-i-1)+(Size-j-1)) * finalVec[Size-j-1];
		}
		finalVec[Size-i-1]=finalVec[Size-i-1]/ *(a_uni_cuda+Size*(Size-i-1)+(Size-i-1));
	}
}
